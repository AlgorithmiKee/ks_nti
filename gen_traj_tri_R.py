...
# generate of sequence of R(t)
def gen_tri_R(prec_r_dB, period_R, T):
    # number of nus and rs
    nus = len(range_nu_db)
    rs = len(range_prec_r_db)

    # convert dB to linear scaling
    r_square_mean = pow(10, -prec_r_dB/10)
    low = r_square_mean*0.5
    high = r_square_mean*1.5
    
    R_tri = torch.empty(size=[dim_y, dim_y, T])
    r_square = gen_tri_wave(low, high, period_R, T) # r2 = r2_mean + A*sin(omega*t), t=1,...,T
    for t in range(0, T):
        R_tri[:,:, t] = r_square[t]*torch.eye(2)
    return R_tri


def generate_traj(NB, F, H, period_R, T, opath) -> None:
    nus = len(range_nu_db)
    rs = len(range_prec_r_db)
    ...
    # stores r(t) for each pair [nu, r]
    R_ALL = torch.empty(size=[nus, rs, NB, T])
    ...
    for idx_nu, nu_db in enumerate(range_nu_db): 
        for idx_r, prec_r_db in enumerate(range_prec_r_db):
            ...
            # periodic R
            R = gen_tri_R(prec_r_db, period_R, T)
            # const Q
    ...
    torch.save(R_ALL, opath+'R_ALL.pt')

# generate and save trajectories
if __name__ == '__main__':
    generate_traj(1000, F, H, period_R=20, T=100, opath='Sim_tri_R/Pd20/traj/')
    generate_traj(1000, F, H, period_R=50, T=100, opath='Sim_tri_R/Pd50/traj/')
    generate_traj(1000, F, H, period_R=100, T=100, opath='Sim_tri_R/Pd100/traj/')
