...
def periodic_R(prec_r_dB, period_R, T):
    R_periodic = torch.empty(size=[dim_y, dim_y, T])
    r_square_mean = pow(10, -prec_r_dB/10)
    amp = r_square_mean/2
    omega_T = 2*pi*torch.arange(0, T)/period_R # omega*t, t=1,...,T
    r_square = r_square_mean + amp*torch.sin(omega_T) # r2 = r2_mean + A*sin(omega*t), t=1,...,T
    for t in range(0, T):
        R_periodic[:,:, t] = r_square[t]*torch.eye(2)
    return R_periodic
...
def generate_traj(NB, F, H, period_R, T, opath) -> None:
    ...
    for idx_nu, nu_db in enumerate(range_nu_db): 
        for idx_r, prec_r_db in enumerate(range_prec_r_db):
            ...
            # periodic R
            R = periodic_R(prec_r_db, period_R, T)
            # const Q
...
if __name__ == '__main__':
    generate_traj(1000, F, H, period_R=20, T=100, opath='Sim_sin_R/Pd20/traj/')
    generate_traj(1000, F, H, period_R=50, T=100, opath='Sim_sin_R/Pd50/traj/')
    generate_traj(1000, F, H, period_R=100, T=100, opath='Sim_sin_R/Pd100/traj/')
