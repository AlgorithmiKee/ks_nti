...
class Extended_rts_smoother:
    ...
    def __init__(self, SystemModel, mode='full'):
        self.f = SystemModel.f # state transition function
        self.m = SystemModel.m # dimension of x

        self.Q = SystemModel.Q

        self.h = SystemModel.h # observation function
        self.n = SystemModel.n # dimension of y

        self.R = SystemModel.R

        self.T = SystemModel.T # time frame
...
class Extended_rts_in(Extended_rts_smoother):
    def __init__(self, SystemModel):
        Extended_rts_smoother.__init__(self, SystemModel)
        self.l = SystemModel.l # input dimension
    ...
    def IGain(self, filter_Q):
        self.IG = torch.matmul(filter_Q, torch.eye(self.m))
        self.IG = torch.matmul(self.IG, torch.inverse(self.filter_sigma_prior))

    # compute input estimate (both 1. and 2. moments)
    def I_Estimate(self, filter_u, filter_Q):
        self.IGain(filter_Q)
        self.u_post = filter_u + torch.matmul(self.IG, self.dx)
        self.Q_post = torch.matmul(self.IG, self.dsigma)
        self.Q_post = self.Q + torch.matmul(self.Q_post, torch.transpose(self.dsigma, 0, 1))

    # combines state smoothing and input estimation
    def SI_Update(self, filter_x, filter_sigma, filter_u, filter_Q):
        x_post, sigma_post = self.S_Update(filter_x, filter_sigma)
        self.IGain(filter_u, filter_Q)
        self.I_Estimate(filter_u, filter_Q)
        return  x_post, sigma_post, self.u_post, self.Q_post

    # process the sequence
    def GenerateSequence(self, filter_x, filter_sigma, filter_u, filter_Q, T):
        # Pre allocate an array for smoothed state and variance
        self.s_x = torch.empty(size=[self.m, T])
        self.s_sigma = torch.empty(size=[self.m, self.m, T])
        # Pre allocate arrays for input estimate and its variance
        self.u_array = torch.empty(size=[self.l, T])
        self.Q_array = torch.empty(size=[self.l, self.l, T])
        # Pre allocate SG and IG arrays
        self.SG_array = torch.zeros((self.T,self.m,self.m))
        self.IG_array = torch.zeros((self.T,self.m,self.l))
        ...
        for t in range(T-2,-1,-1):
            ...
            s_xt,s_sigmat, ut, Qt = self.SI_Update(filter_xt, filter_sigmat, filter_ut, filter_Qt)
            self.s_x[:, t] = torch.squeeze(s_xt)
            self.s_sigma[:, :, t] = torch.squeeze(s_sigmat)
            self.u_array[:, :, t] = torch.squeeze(ut)
            self.Q_array[:, :, t] = torch.squeeze(Qt)