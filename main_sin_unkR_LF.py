...
from NUV_test import Unknown_R_LF_Tester
...
def test_batch(nu_r):
    ...
    tester = Unknown_R_LF_Tester(lin_model)
    # idx for trace back
    idices = [idx_nu, idx_r]
    # test
    MSE_and_std = tester.test(NB, test_input, X_target, U_target, init_unknown=r, itr=40, forget_itr=0.9)     
    # save estimates
    ...
    for idx_nu, nu_db in enumerate(range_nu_db): 
        for idx_r, prec_r_db in enumerate(range_prec_r_db): 
            ...
            [
                ...
                MSEs_input_unknownR[idx_nu, idx_r],
                std_MSE_input_unknownR[idx_nu, idx_r]
            ] = tester.test(NB, test_input, X_target, U_target, init_unknown=r, itr=20, forget_itr=0.0)     
...
def save_plot(opath, bpath):
    ...
    ##########################################
    plt.figure()
    plt.title(r'State estimation (unknown sinusoidal $r(t)$)')
    ...
    MSEs_state_baseline = torch.load(bpath+'MSE_state_unknownR.pt')
    ...
    ##########################################
    plt.figure()
    plt.title(r'Input estimation (unknown sinusoidal $r(t)$)')
    plt.xlabel(r'$\frac{1}{r^2}$ in dB')
    plt.ylabel('MSE of input estimation in dB')
    # Noise floor
    plt.plot(range_prec_r_db, [ -x for x in range_prec_r_db], '--', c='r', linewidth=0.75, label='Noise floor')
    # load & plot MSE for base line
    MSEs_input_baseline = torch.load(bpath+'MSE_input_unknownR.pt')
...
if __name__ == '__main__':
    main(opath='Sim_sin_R/Pd100/LF/', bpath='Sim_sin_R/Pd100/MA/')
