...
from Simple_NUV_test import Unknown_Q_Tester
...
def test_batch(nu_r):
    ...
    MSE_and_std = tester.test(NB, test_input, X_target, U_target, itr=100, init_unknown=1.0)
    # save estimates
...
def main_sp(opath, bpath):   
    ...
    for idx_nu, nu_db in enumerate(range_nu_db): 
        for idx_r, prec_r_db in enumerate(range_prec_r_db): 
            ...
            ######################################################################       
            # case 2: asuming that our model don't know the true Q (but true Q)  #
            ######################################################################     
            test_input = Y_ALL[idx_nu, idx_r, :, :, :]
            ################################################
            # Evaluate the input estimator over NB batches #
            ################################################
            test_input = Y_ALL[idx_nu, idx_r, :, :, :]
            U_target = U_ALL[idx_nu, idx_r, :, :, :]
            X_target = X_ALL[idx_nu, idx_r, :, :, :]
            ...
            [
                ...
                MSEs_input_unknownQ[idx_nu, idx_r],
                std_MSE_input_unknownQ[idx_nu, idx_r]
            ]= tester.test(NB, test_input, X_target, U_target, itr=80, init_unknown=1.0 )
    ...
    print("Total Time - single processing:", time.time()-start)
    save_data(opath)
    save_plot(opath, bpath)
...
def save_plot(opath, bpath):
    ##########################################
    #### Plot MSE of input for unknown Q #####
    ##########################################
    plt.figure()
    plt.title('Input estimation (unknown static Q)')
    ...
    plt.plot(range_prec_r_db, MSEs_input_unknownQ[0, :], '*-', c='c', linewidth=0.75, label=r'$\nu$ =   0 dB, unknown $Q$')
    plt.plot(range_prec_r_db, MSEs_input_unknownQ[1, :], 'o-', c='b', linewidth=0.75, label=r'$\nu$ = -10 dB, unknown $Q$')
    plt.plot(range_prec_r_db, MSEs_input_unknownQ[2, :], '^-', c='g', linewidth=0.75, label=r'$\nu$ = -20 dB, unknown $Q$')

    plt.legend(loc="upper right")
    plt.grid()
    plt.savefig(opath+'MSE input unknown q.pdf')
    ...
    ##########################################
    plt.figure()
    plt.title('State estimation (unknown static Q)')
    plt.xlabel(r'$\frac{1}{r^2}$ in dB')
    ...
    # plot MSE for simple nuv
    plt.plot(range_prec_r_db, MSEs_state_unknownQ[0, :], '*-', c='c', linewidth=0.75, label=r'$\nu$ =   0 dB, unknown $Q$')
    plt.plot(range_prec_r_db, MSEs_state_unknownQ[1, :], 'o-', c='b', linewidth=0.75, label=r'$\nu$ = -10 dB, unknown $Q$')
    plt.plot(range_prec_r_db, MSEs_state_unknownQ[2, :], '^-', c='g', linewidth=0.75, label=r'$\nu$ = -20 dB, unknown $Q$')

    plt.legend(loc="upper right")
    plt.grid()
    plt.savefig(opath+'MSE state unknown q.pdf')

    plt.show()


if __name__ == '__main__':
    main(opath='Sim_const_Q/', bpath='Sim_baseline/KS/')